...
it('verify Maharashtra region/state in India has holidays', function () {
    expect(Holidays::for(India::make('Maharashtra'),)->get())
        ->toBeArray()
        ->not()->toBeEmpty();
});

it('verify Maharashtra holiday is not found in other region', function () {
    expect(Holidays::for(India::make('Maharashtra'))->getName(date: '2024-04-13'))->toBe('Gudi Padwa');
    expect(Holidays::for(India::make('Karnataka'))->getName(date: '2024-04-13'))->not->toBe('Gudi Padwa');
});

it('verify wrong region throws exception in India', function () {
    expect(Holidays::for(India::make('XYZ'))->get())->not->toBeArray();
